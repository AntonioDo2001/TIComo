...
export class CrearPedidoComponent implements OnInit {
  ...
  agregarPedido(){
    if(this.pedidoForm.get('cantidadPlato')?.value < 1){
      this.toastr.error('La cantidad del pedido debe ser mayor o igual a 1', 'CANTIDAD ERRONEA');
    }
    else{
      this._platoService.obtenerPlato(this.idPlato).subscribe(data =>{
        this.nombreRestaurante = data.nombreRestaurante;
        var PEDIDO = new Pedido(this.pedidoForm.get('nombrePlato')?.value,this.pedidoForm.get('precioPlato')?.value,this.pedidoForm.get('cantidadPlato')?.value,this.idCliente,this.nombreRestaurante)
